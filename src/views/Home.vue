<template>
  <div>
    ...
    <!-- Promotions Section -->
    <section v-if="activePromotions.length > 0" class="py-20 bg-gradient-to-r from-red-50 to-pink-50">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
          <h2 class="font-title font-bold text-3xl lg:text-4xl text-gray-900 mb-4">
            🔥 Offres spéciales
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div 
            ...
            :key="promotion.id"
            class="bg-white rounded-2xl shadow-lg overflow-hidden group hover:shadow-xl transition-shadow duration-300"
          >
            <div class="relative">
              <img 
                :src="promotion.image" 
              ...
              <div v-if="promotion.promoCode" class="absolute bottom-4 left-4">
                <span class="bg-black/80 text-white px-3 py-1 rounded-lg text-xs font-mono">
                  {{ promotion.promoCode }}
                </span>
              </div>
            </div>
            <div class="p-6">
              <h3 class="font-title font-bold text-xl text-gray-900 mb-2">
                {{ promotion.title }}
              </h3>
...
const activePromotions = computed(() => promotionStore.activePromotions)
...
const sharePromotion = (promotion: any) => {
  const message = `🔥 *Promotion Spéciale - Merry's Food* 🔥\n\n` +
    `*${promotion.title}*\n` +
    `${promotion.description}\n\n` +
    ...
    `Découvrez tous nos produits sur notre site.`
